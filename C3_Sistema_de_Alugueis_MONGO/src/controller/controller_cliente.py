...
class Controller_Cliente:
    ...
    def excluir_cliente(self):
        ...
        while aux_loop == "s":
            ...
            if not self.verifica_existencia_cliente(cpf):            
                ...
                if not self.verifica_existencia_cliente_em_alugueis(cpf):
    ...
    def verifica_existencia_cliente_em_alugueis(self, cpf:str=None, external:bool=False) -> bool:
        if external:
            # Cria uma nova conexão com o banco que permite alteração
            self.mongo.connect()

        # Recupera os dados do novo cliente criado transformando em um DataFrame
        df_cliente = pd.DataFrame(self.mongo.db["alugueis"].find({"cpf":f"{cpf}"}, {"cpf": 1, "nome": 1, "_id": 0}))
